 ...
 */ class GLTFMaterialsSpecularExtension {
        ...
        extendMaterialParams(materialIndex, materialParams) {
            ...
            if (extension.specularColorTexture !== undefined) pending.push(parser.assignTexture(materialParams, "specularColorMap", extension.specularColorTexture, THREE.sRGBEncoding));
            return Promise.all(pending);
        }
    }
    /**
 ...
 */ function updateMorphTargets(mesh, meshDef) {
    ...
    /* GLTF PARSER */ class GLTFParser {
        constructor(json = {}, options = {}){
            ...
            if (typeof createImageBitmap !== "undefined" && /^((?!chrome|android).)*safari/i.test(navigator.userAgent) === false) this.textureLoader = new THREE.ImageBitmapLoader(this.options.manager);
            else this.textureLoader = new THREE.TextureLoader(this.options.manager);
   ...
   */ loadTexture(textureIndex) {
        ...
        loadImageSource(sourceIndex, loader) {
            ...
            let sourceURI1 = sourceDef.uri || "";
            let isObjectURL = false;
            if (sourceDef.bufferView !== undefined) // Load binary image data from bufferView, if provided.
            sourceURI1 = parser.getDependency("bufferView", sourceDef.bufferView).then(function(bufferView) {
                isObjectURL = true;
                const blob = new Blob([
                    bufferView
                ], {
   ...
   */ loadMaterial(materialIndex) {
        ...
        /** When THREE.Object3D instances are targeted by animation, they need unique names. */ createUniqueName(originalName) {
   ...
   */ loadMesh(meshIndex) {
            ...
            for(let i5 = 0, il4 = primitives.length; i5 < il4; i5++){
                const material = primitives[i5].material === undefined ? createDefaultMaterial(this.cache) : this.getDependency("material", primitives[i5].material);
   ...
   */ loadScene(sceneIndex) {
            ...
            return Promise.all(pending).then(function() {
                ...
                const reduceAssociations = (node1)=>{
                    ...
                    for (const [key, value] of parser.associations)if (key instanceof THREE.Material || key instanceof THREE.Texture) reducedAssociations.set(key, value);
                    node1.traverse((node)=>{
...
//# sourceMappingURL=index.75ba786e.js.map
